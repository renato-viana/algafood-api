...
public interface ProdutoControllerOpenApi {
    ...
    CollectionModel<ProdutoModelResponse> listar(
            ...
            @Parameter(description = "Indica se deve ou não incluir produtos inativos no resultado da listagem",
                    example = "false", required = false) Boolean incluirInativos
    );
    ...
    ProdutoModelResponse adicionar(
            @Parameter(description = "ID do restaurante", example = "1", required = true) Long restauranteId,
            @RequestBody(description = "Representação de um novo produto", required = true) ProdutoModelRequest produtoModelRequest);
    ...
    ProdutoModelResponse atualizar(
            @Parameter(description = "ID do restaurante", example = "1", required = true) Long restauranteId,
            @Parameter(description = "ID do produto", example = "1", required = true) Long produtoId,
            @RequestBody(description = "Representação de um produto com os novos dados", required = true) ProdutoModelRequest produtoModelRequest);
